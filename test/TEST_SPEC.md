...
redmine_date_independent プラグインのテスト仕様。
...
**確認方法:**
```ruby
plugin = Redmine::Plugin.find(:redmine_date_independent)
puts plugin.name
puts plugin.version
```
...
**確認方法:**
```ruby
puts Issue.included_modules.include?(RedmineDateIndependent::IssuePatch)
```

**期待結果:** `true` が出力される

### [1-6] DateIndependent モデル確認
